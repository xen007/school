...
const calculateAverages = () => {
  ...
  Object.keys(groupedData).forEach(matricule => {
      let totalNote1 = 0, totalNote2 = 0, totalmoy = 0, totalMoy1 = 0, totalMoy2 = 0, totalMoy3 = 0, totalGmoy = 0;
      let totalCoef1 = 0, totalCoef2 = 0, totalCoef3 = 0, totalMoyCoef = 0;
      ...
      Object.keys(studentData).forEach(matiere => {
          studentData[matiere].forEach(item => {
              ...
              if (!isNaN(note1) && note1 !== null && !isNaN(coef) && coef !== null) {
                  totalNote1 += note1 * coef;
                  totalCoef1 += coef;
              }
              if (!isNaN(note2) && note2 !== null && !isNaN(coef) && coef !== null) {
                  totalNote2 += note2 * coef;
                  totalCoef2 += coef;
              }
              if (!isNaN(moy) && moy !== null && !isNaN(coef) && coef !== null) {
                  totalmoy += moy * coef;
                  totalMoyCoef += coef;
              }
              if (!isNaN(moy1) && moy1 !== null && !isNaN(coef) && coef !== null) {
                  totalMoy1 += moy1 * coef;
                  totalCoef1 += coef;
              }
              if (!isNaN(moy2) && moy2 !== null && !isNaN(coef) && coef !== null) {
                  totalMoy2 += moy2 * coef;
                  totalCoef2 += coef;
              }
              if (!isNaN(moy3) && moy3 !== null && !isNaN(coef) && coef !== null) {
                  totalMoy3 += moy3 * coef;
                  totalCoef3 += coef;
              }
              if (!isNaN(Gmoy) && Gmoy !== null && !isNaN(coef) && coef !== null) {
                  totalGmoy += Gmoy * coef;
                  totalMoyCoef += coef;
              }
          });
      ...
      const averageNote1 = totalCoef1 === 0 ? '/' : (totalNote1 / totalCoef1).toFixed(2);
      const averageNote2 = totalCoef2 === 0 ? '/' : (totalNote2 / totalCoef2).toFixed(2);
      const averageMoy = totalMoyCoef === 0 ? '/' : (totalmoy / totalMoyCoef).toFixed(2);
      const averageMoy1 = totalCoef1 === 0 ? '/' : (totalMoy1 / totalCoef1).toFixed(2);
      const averageMoy2 = totalCoef2 === 0 ? '/' : (totalMoy2 / totalCoef2).toFixed(2);
      const averageMoy3 = totalCoef3 === 0 ? '/' : (totalMoy3 / totalCoef3).toFixed(2);
      const averageGmoy = totalMoyCoef === 0 ? '/' : (totalGmoy / totalMoyCoef).toFixed(2);
      ...
      newAverages[matricule] = {
          ...
          appreciationMoy1: getAppreciation(averageMoy1),
          appreciationMoy2: getAppreciation(averageMoy2),
          appreciationMoy3: getAppreciation(averageMoy3),
...
const assignRanks = () => {
  ...
  sortedStudents.forEach((matricule, index) => {
      averages[matricule].rank = index + 1;
  });
