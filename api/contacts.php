...
//echo "test-----".$method; die;
    ...
    switch($method){
        case "GET"://methode de récupération de données
            ...
            $getuserrow=mysqli_query($db_connect, "SELECT *, SUM((COALESCE(note1,note2) +COALESCE(note2,note1))/2 * coefficient) AS val, SUM(coefficient) AS coef FROM note INNER JOIN eleve on note.matricule_El = eleve.matricule_El INNER JOIN matiere on note.id_matiere = matiere.id_matiere INNER JOIN classe on eleve.classe = classe.id_classe LEFT JOIN enseignant on matiere.matricule_Ens = enseignant.matricule_Ens group by note.matricule_El order by val asc ");
            if(mysqli_num_rows($getuserrow) > 0){
                   ...
                   while($userrow = mysqli_fetch_array($getuserrow) ){
                    ...
                    //     $json_array['rowUserdata'][]= array( 'matricule' =>$userrow['matricule_El'],'nom' =>$userrow['nom'],'genre' =>$userrow['genre'],'prénom' =>$userrow['prénom'],'id' =>$userrow['id_note'], 'idnote' =>$userrow['id_note'],
                    //     'idmatiere' =>$userrow['id_matiere'],'coef' =>$userrow['coefficient'],'note1' =>$userrow['note1'],'note2' =>$userrow['note2'],'nomMat' =>$userrow['nom_matiere'],'nomEn' =>$userrow['nomE'], 'app'=> $userrow['val']
                    //  );
                        $json_array['rowUserdata'][]= array( 'matricule' =>$userrow['matricule_El'], 'ap'=> $userrow['val'],'moy'=> $moy,'note1' => $userrow['note1'],
                     );
                    }
